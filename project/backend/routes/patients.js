...
router.post("/resend-verification", async (req, res) => {
  try {
    ...
    await db.queryAsync(
      `UPDATE users
       SET verification_token = ?, verification_expires_at = NOW() + INTERVAL '24 hours'
       WHERE userId = ?`,
  ...
  try {
    ...
    if (userRows.length === 0) {
      ...
      if (legacyPatients.length > 0) {
        ...
        await db.queryAsync(
          `INSERT INTO users (fullName, email, password, role, linked_patient_id, email_verified)
           VALUES (?, ?, ?, 'Patient', ?, 1) ON CONFLICT DO NOTHING`,
...
router.get("/", authenticate, requireRole("Admin"), async (req, res) => {
  try {
    ...
    const nowP = new Date();
    const todayP = nowP.getFullYear() + "-" + String(nowP.getMonth() + 1).padStart(2, "0") + "-" + String(nowP.getDate()).padStart(2, "0");
    ...
    const rows = await db.queryAsync(
      `SELECT p.id, p.full_name, p.email, p.phone, p.ic_number,
       ...
       LIMIT ? OFFSET ?`,
      [...params, todayP, todayP, todayP, todayP, pageSize, offset]
    );
  ...
  try {
    ...
    const rows = await db.queryAsync(
      `SELECT id, full_name, email, phone, ic_number, date_of_birth, gender, created_at,
              EXTRACT(YEAR FROM AGE(CURRENT_DATE, date_of_birth)) AS age, needs_follow_up
