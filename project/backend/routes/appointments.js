  ...
  try {
    ...
    const insertResult = await db.queryAsync(
      `INSERT INTO appointments
       (patient_name, ic_number, patient_email, patient_phone, doctor_id, appointment_date, appointment_time, queue_number, status, description, patient_id, check_in_token, check_in_confirmed, created_at)
    ...
    const appointmentData = {
      appointmentId,
      doctorId: value.doctor_id,
      doctorName: doctorRows[0]?.name || "Assigned Doctor",
  ...
  try {
    ...
    const results = await db.queryAsync(
      `SELECT a.*, d.name AS doctor_name,
              mr.diagnosis, mr.prescriptions, mr.notes AS medical_record_notes,
              mr.visit_date AS medical_record_date,
  ...
  try {
    ...
    const rows = await db.queryAsync(
      `SELECT a.id, a.patient_name, a.appointment_date, a.appointment_time, a.queue_number,
       ...
       FROM appointments a
       LEFT JOIN doctors d ON d.id = a.doctor_id
       WHERE a.check_in_token = ? AND a.appointment_date = ?
       LIMIT 1`,
      [token, todayStr]
    );

    if (rows.length === 0) {
      const altRows = await db.queryAsync(
        "SELECT appointment_date FROM appointments WHERE check_in_token = ? LIMIT 1",
        [token]
      );
      if (altRows.length > 0) {
        return res.status(400).json({
          success: false,
          message: `This token belongs to an appointment on ${altRows[0].appointment_date}. Please check in on that date.`
        });
      }
    ...
    const statusNote = appointment.status === "Completed"
      ? "Your appointment has already been completed."
...
router.post("/checkin/:token?", async (req, res) => {
  try {
    ...
    const statusNote2 = appointment.status === "Completed"
      ? "Your appointment has already been completed."
      : appointment.status === "In Progress"
        ? "You are currently being attended to."
        : "Check-in confirmed";
