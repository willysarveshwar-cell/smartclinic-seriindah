...
async function saveSession(userId, refreshToken) {
  await db.queryAsync(
    `UPDATE users
     SET refresh_token = ?, refresh_expires_at = NOW() + INTERVAL '7 days', last_login_at = NOW()
     WHERE userId = ?`,
    [refreshToken, userId]
  );
}
...
async function loginByRole(req, res, role) {
  try {
    ...
    if (users.length === 0 && role === "Admin" && email === "admin@clinic.com" && password === "admin123") {
      await db.queryAsync(
        "INSERT INTO users (fullName, email, password, role, email_verified) VALUES ('Admin User', 'admin@clinic.com', 'admin123', 'Admin', 1) ON CONFLICT DO NOTHING"
      );
...
router.post("/doctor/login", async (req, res) => {
  try {
    ...
    if (doctorRows.length === 0) {
      await logAuthEvent({ req, eventType: "DOCTOR_LOGIN", status: "FAILED", email, role: "Doctor", details: { reason: "doctor_not_found" } });
      return res.status(401).json({ message: "Invalid doctor credentials" });
    }

    const doctorRecord = doctorRows[0];
    const valid = await comparePassword(password, doctorRecord.password);

    if (!valid) {
      await logAuthEvent({ req, eventType: "DOCTOR_LOGIN", status: "FAILED", email, role: "Doctor", details: { reason: "invalid_password" } });
      return res.status(401).json({ message: "Invalid doctor credentials" });
    }

    if (!isBcryptHash(doctorRecord.password)) {
      const hashed = await hashPassword(password);
      await db.queryAsync("UPDATE doctors SET password = ? WHERE id = ?", [hashed, doctorRecord.doctorId]);
      doctorRecord.password = hashed;
    }

    let userRows = await db.queryAsync(
      `SELECT userId, fullName, email, password, role, email_verified
       FROM users
       WHERE LOWER(email) = ?
       LIMIT 1`,
      [email]
    ...
    if (!user) {
      await db.queryAsync(
        "INSERT INTO users (fullName, email, password, role, email_verified) VALUES (?, ?, ?, 'Doctor', 1) ON CONFLICT DO NOTHING",
        [doctorRecord.name, doctorRecord.email, doctorRecord.password]
      );
      const insertedUsers = await db.queryAsync(
        "SELECT userId, fullName, email, password, role, email_verified FROM users WHERE LOWER(email) = ? LIMIT 1",
        [email]
      );
    ...
    if (user && String(user.role).toLowerCase() !== "doctor") {
      await db.queryAsync("UPDATE users SET role = 'Doctor' WHERE userId = ?", [user.userId]);
      user.role = "Doctor";
    }

    if (!isBcryptHash(user.password) || (user.password && doctorRecord.password && user.password !== doctorRecord.password)) {
      await db.queryAsync("UPDATE users SET password = ? WHERE userId = ?", [doctorRecord.password, user.userId]);
      user.password = doctorRecord.password;
    }

    const doctorId = doctorRecord.doctorId;
    const token = signAccessToken({ id: user.userId, role: "Doctor", email: user.email, doctorId });
    const refreshToken = signRefreshToken({ id: user.userId, role: "Doctor", email: user.email, doctorId });
    await saveSession(user.userId, refreshToken);

    await logAuthEvent({ req, eventType: "DOCTOR_LOGIN", status: "SUCCESS", userId: user.userId, email: user.email, role: "Doctor", details: { doctorId } });
