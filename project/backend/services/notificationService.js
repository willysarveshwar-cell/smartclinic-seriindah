  ...
  const reminderRows = await db.queryAsync(
    `SELECT a.id, a.doctor_id, a.queue_number, a.status, a.appointment_date, a.appointment_time,
     ...
     WHERE a.status = 'Waiting'
       ...
       AND EXTRACT(EPOCH FROM ((a.appointment_date || ' ' || a.appointment_time)::timestamp - NOW()))/60 BETWEEN 0 AND ?`,
  ...
  const missedRows = await db.queryAsync(
    `SELECT a.id, a.doctor_id, a.queue_number, a.status, a.appointment_date, a.appointment_time,
     ...
     WHERE a.status = 'Waiting'
       ...
       AND EXTRACT(EPOCH FROM (NOW() - (a.appointment_date || ' ' || a.appointment_time)::timestamp))/60 >= ?`,
