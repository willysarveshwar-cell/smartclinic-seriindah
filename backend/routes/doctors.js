...
router.get("/", (req, res) => {
  ...
  db.query(sql, params, (err, results) => {
    if (err) {
      console.error("❌ Get doctors SQL error:", err.sqlMessage);
...
router.get("/:doctorId/appointments/today", authenticate, requireRole("Doctor", "Admin"), (req, res) => {
  ...
  const sql = `
    ...
    FROM appointments
    WHERE doctor_id = ?
      AND appointment_date >= DATE_FORMAT(CURDATE(), '%Y-%m-%d')
  ...
  db.query(sql, [doctorId], (err, results) => {
    if (err) {
      console.error("❌ Today appointments SQL error:", err.sqlMessage);
      return res.status(500).json({ message: "Database error" });
    }
...
router.get("/:doctorId/queue", authenticate, requireRole("Doctor", "Admin"), (req, res) => {
  ...
  db.query(sql, [doctorId], (err, results) => {
    if (err) {
      console.error("❌ Queue list SQL error:", err.sqlMessage);
      return res.status(500).json({ message: "Database error" });
    }
...
router.put("/:doctorId/availability", authenticate, requireRole("Doctor", "Admin"), async (req, res) => {
  try {
    ...
    for (const slot of normalizedSlots) {
      ...
      await db.queryAsync(
        `INSERT IGNORE INTO doctor_availability (doctor_id, day_of_week, start_time, end_time, is_active)
         VALUES (?, ?, ?, ?, ?)`,
...
router.put("/appointment/:appointmentId/status", authenticate, requireRole("Doctor", "Admin"), (req, res) => {
  ...
  db.query(sql, params, (err, result) => {
    if (err) {
      console.error("❌ Update status SQL error:", err.sqlMessage);
      return res.status(500).json({ message: "Database error" });
    }
...
router.put("/appointment/:appointmentId/description", authenticate, requireRole("Doctor", "Admin"), (req, res) => {
  ...
  db.query(sql, [description, appointmentId], (err, result) => {
    if (err) {
      console.error("❌ Update description SQL error:", err.sqlMessage);
      return res.status(500).json({ message: "Database error" });
    }
...
router.get("/appointment/:appointmentId", authenticate, requireRole("Doctor", "Admin"), (req, res) => {
  ...
  db.query(sql, [appointmentId], (err, results) => {
    if (err) {
      console.error("❌ Get appointment SQL error:", err.sqlMessage);
      return res.status(500).json({ message: "Database error" });
    }
...
router.put("/:id", authenticate, requireRole("Admin"), async (req, res) => {
  try {
    ...
    const existingRows = await db.queryAsync("SELECT email FROM doctors WHERE id = ? LIMIT 1", [doctorId]);
    ...
    await db.queryAsync(
      "UPDATE doctors SET name = ?, specialization = ?, email = ? WHERE id = ?",
      [name, specialization, normalizedEmail, doctorId]
    );

    // Keep auth identity in sync so doctor login keeps working after profile edits.
    await db.queryAsync(
      "UPDATE users SET fullName = ?, email = ? WHERE LOWER(email) = ? AND LOWER(role) = 'doctor'",
      [name, normalizedEmail, previousEmail]
    );
